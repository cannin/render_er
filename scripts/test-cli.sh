...
expected_version="$(awk -F ' = ' '$1 == "version" { gsub(/"/, "", $2); print $2; exit }' "$repository_root/rust/Cargo.toml")"
...
assert_draw_options() {
  ...
  for option in \
    ...
    --generate-render-test-manifest; do
...
assert_equivalent_help Rust \
  "$repository_root/rust/target/debug/render_er"
assert_equivalent_help Go "$temporary_directory/render_sbgn_go"
assert_equivalent_help R run_r_cli

assert_version Python \
  uv run --project "$repository_root/python" render_sbgn_py
assert_version Rust "$repository_root/rust/target/debug/render_er"
assert_version Go "$temporary_directory/render_sbgn_go"
assert_version R run_r_cli
...
assert_equivalent_help "Rust draw_sbgnml" \
  "$repository_root/rust/target/debug/render_er" draw_sbgnml
